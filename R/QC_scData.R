...
#'
#' @param List The "Simana" object that was built after run "Read_scRNA" function. \cr
...
QC_scData <- function(List, Gene_threshold = 0, Count_threshold = 0, Cell_threshold = 0, MT_threshold = 0) {
  ...
  Data_bin <- as.numeric(Data_a > 0)
  dim(Data_bin) <- dim(Data_a)
  Max_count = max(Data_a)
  Items_used = Data_a > 0
  List_1 <- list(
